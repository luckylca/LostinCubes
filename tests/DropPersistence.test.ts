...
describe('ground drop persistence', () => {
  it('round-trips block and material drops by world seed', () => {
    const storage = new MemoryStorage();
    const snapshots = [
      { item: ItemType.OakLogBlock, count: 5, x: 3.25, y: 7.14, z: -2.5, grounded: true },
      { item: ItemType.RawIron, count: 2, x: -4, y: 5, z: 9, grounded: false },
    ] as const;
  ...
  it('migrates legacy block drops and filters malformed snapshots', () => {
    const storage = new MemoryStorage();
    storage.setItem(
      'lost-in-cubes:drops:world-a',
      JSON.stringify([
        { block: BlockType.OakPlanks, count: 999, x: 1, y: 2, z: 3, grounded: true },
        { item: ItemType.Coal, count: 3, x: 4, y: 5, z: 6, grounded: false },
    ...
    expect(loadDroppedItems('world-a', storage)).toEqual([
      { item: ItemType.OakPlanksBlock, count: 64, x: 1, y: 2, z: 3, grounded: true },
      { item: ItemType.Coal, count: 3, x: 4, y: 5, z: 6, grounded: false },
    ]);
  });
