...
describe('crafting recipes', () => {
  it('keeps personal crafting limited to planks, sticks, and a table', () => {
    const personal = getVisibleRecipes(false);

    expect(personal.map((recipe) => recipe.id)).toEqual([
      'oak-planks',
      'sticks',
      'crafting-table',
    ]);
    expect(getVisibleRecipes(true)).toHaveLength(CRAFTING_RECIPES.length);
  });
  ...
  it('supports the log to stone-tool progression with inventory consumption', () => {
    ...
    if (planks === undefined || sticks === undefined || stonePickaxe === undefined) {
      return;
    }
