...
test('boots survival progression, crafts, and preserves camera controls', async ({
  page,
}) => {
  ...
  await page.addInitScript(() => {
    ...
    const slots: SeedSlot[] = Array.from({ length: 36 }, () => ({ item: null, count: 0, durability: null }));
    ...
    slots[28] = { item: 'furnace-block', count: 1, durability: null };
  ...
  await expect(page.locator('html')).toHaveAttribute('data-game-state', 'ready', { timeout: 45_000 });
  ...
  await expect(page.locator('#hotbar .hotbar-slot').first()).toHaveAttribute('aria-label', /铁镐.*耐久 250\/250/);
  ...
  await expect(canvas).toHaveAttribute('data-player-health', '20');
  await expect(canvas).toHaveAttribute('data-death-count', '0');
  await expect(canvas).toHaveAttribute('data-inventory-open', 'false');
  await expect(page.locator('#hud-status')).toContainText('生命 20/20');
  const startingDayTime = Number(await canvas.getAttribute('data-day-time'));
  expect(startingDayTime).toBeGreaterThan(0);
  expect(startingDayTime).toBeLessThan(1);
  await expect
  ...
  await expect(page.locator('[data-inventory-index="4"]')).toHaveAttribute('aria-label', /煤炭 × 3/);
  await expect(page.locator('[data-inventory-index="5"]')).toHaveAttribute('aria-label', /粗铁 × 2/);
  await expect(page.locator('[data-inventory-index="6"]')).toHaveAttribute('aria-label', /铁锭 × 3/);
  ...
  await expect(page.locator('[data-inventory-index="7"]')).toHaveAttribute('aria-label', /橡木木板 × 4/);
  ...
  if (canvasBounds === null) throw new Error('Canvas bounds were unavailable.');
  ...
  await expect.poll(async () => Number(await canvas.getAttribute('data-player-pitch'))).toBeLessThan(-1.5);
...
test('falls back to synchronous terrain when module workers fail', async ({ page }) => {
  const runtimeErrors: string[] = [];
  ...
  await expect(page.locator('#loading-screen')).toHaveClass(/is-hidden/);
  await expect(page.locator('#hotbar .hotbar-slot')).toHaveCount(9);
  await expect(page.locator('#game-canvas')).toHaveAttribute('data-player-health', '20');
  expect(runtimeWarnings.some((message) => message.includes('Chunk worker failed at runtime; continuing with synchronous generation.'))).toBe(true);
  expect(runtimeErrors).toEqual([]);
});
