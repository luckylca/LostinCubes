...
export function installTerrainVarietyRuntime(): void {
  ...
  // These references must retain the TerrainGenerator receiver when invoked.
  ...
  TerrainGenerator.prototype.sampleSurfaceHeight = function variedHeight(
    ...
    const ridge =
      1 -
      Math.abs(
        valueNoise(worldX, worldZ, 27, seed ^ 0xc2b2ae35) * 2 - 1,
      );

    const spawnDistance = Math.hypot(worldX, worldZ - 3.5);
    const spawnBlend = Math.min(Math.max((spawnDistance - 14) / 22, 0), 1);
    const hillWeight = smoothStep(
      Math.min(Math.max((regional - 0.5) / 0.36, 0), 1),
    );
    const relief =
      (rolling - 0.5) * 3.4 +
      (detail - 0.5) * 1.35 +
